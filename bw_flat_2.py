...
InitialValues = np.zeros(N*4).astype(np.float32)
...
def BWA_GPU_RHS(stvars_h, args):
    ...
    stdrvs_h = np.empty_like(stvars_h) #CPU result buffer
...
def euler(RHS, stvars, h, args):
    stdrvs = RHS(stvars, args)
    return stvars + h*stdrvs
...
V = np.zeros((N, len(time))).astype(np.float32)
stvars = InitialValues
start_time = time.time()
for i in range(len(time)):
    V[:,i] = stvars[:N]
    stvars = BWA_GPU_RHS(stvars, args)
    if i % 1000 == 0:
        print i
...
print "sim time: ", stop_time - start_time
